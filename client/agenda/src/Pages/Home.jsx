...
const Home = () => {
    ...
    const getEntidades = async () => {
        const response = await axios.get(`http://181.204.15.130:5000/api/entidades`)
    ...
    const getEspecialistas = async () => {
        const response = await axios.get(`http://181.204.15.130:5000/api/especialistas`)
    ...
    const getFechaAgenda = async () => {
        const response = await axios.get(`http://181.204.15.130:5000/api/agenda?fecha=${datos[datos.length-1].fecha}&contrato=${datos[datos.length-1].contrato}&especialista=${datos[datos.length-1].especialista}`)
    ...
    const cboFecDis_onChange = () => {
        ...
        //--------HORAS DISPONIBLES -------
        axios.get(`http://181.204.15.130:5000/api/hora?fecha=${cboFechaDisponible.current.value}&contrato=${document.getElementById('idContrato').value}&especialista=${document.getElementById('idEspecialista').value}`).then((response)=>{
    ...
    const btnAsignarCita_Click=(id) => {
        ...
        //--------DATOS CITA -------
        axios.get(`http://181.204.15.130:5000/api/cita/${id}`).then((response)=>{
    ...
    const btnCita_Click=(id)=>{
        ...
        //-------CONSULTAR PACIENTE -------
        axios.get(`http://181.204.15.130:5000/api/persona?tipoidentificacion=${document.getElementById('idTipoIdentificacion').value}&identificacion=${id}`).then((response)=>{
    ...
    const Boton=(e)=>{
        ...
        if (contrato=== "" || especialista==="" || fecha==="") {        
            ...
            //--------FECHAS DISPONIBLES -------
            axios.get(`http://181.204.15.130:5000/api/agenda?fecha=${document.getElementById('dtFecha').value}&contrato=${document.getElementById('idContrato').value}&especialista=${document.getElementById('idEspecialista').value}`).then((response)=>{
                if (response.data.length===0){
                    salert('true')
                    ResultadoConsulta.current.className="d-none";
                }else{
            ...
            //--------HORAS DISPONIBLES -------
            axios.get(`http://181.204.15.130:5000/api/hora?fecha=${document.getElementById('dtFecha').value}&contrato=${document.getElementById('idContrato').value}&especialista=${document.getElementById('idEspecialista').value}`).then((response)=>{
                if (response.data.length===0){
